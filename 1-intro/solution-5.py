...
def play(sufficient_cnt):
	...
	while money > 0 and money < bandit.celling():
		...
		def sample_an_arm():
			...
			mature_ones = []
			for arm in range(0, arm_num):
				if win[arm] >= sufficient_cnt and prior[arm][0] < prior[arm][1]:
					continue
				if win[arm] >= sufficient_cnt and prior[arm][0] > prior[arm][1]:
...
def main():
	...
	for sufficient_cnt in range(5, 41):
		win = 0
		for i in range(0, 10000):
			win += play(sufficient_cnt)
		res.append([sufficient_cnt, win])

	for one_tuple in res:
		print one_tuple[0], ':', one_tuple[1]
